...
class FiltersParsingTest extends FunSuite {
  ...
  val ec2TagBasedSimpleServiceDiscovery = new Ec2TagBasedSimpleServiceDiscovery(null)

  import scala.collection.JavaConverters._

  test("empty string does not break parsing") {
    val filters = ""
    val result = ec2TagBasedSimpleServiceDiscovery.parseFiltersString(filters)
    assert(result.isEmpty)
  }

  test("can parse simple filter") {
    val filters = "tag:purpose=demo"
    val result: List[Filter] = ec2TagBasedSimpleServiceDiscovery.parseFiltersString(filters)
