...
private[akka] object KubernetesSettings {
  ...
  def apply(config: Config, leaseTimeoutSettings: TimeoutSettings): KubernetesSettings = {
    ...
    val apiServerRequestTimeout = if (config.hasDefined("api-server-request-timeout"))
    ...
    require(apiServerRequestTimeout < leaseTimeoutSettings.operationTimeout, "'api-server-request-timeout can not be less than 'lease-operation-timeout'")
