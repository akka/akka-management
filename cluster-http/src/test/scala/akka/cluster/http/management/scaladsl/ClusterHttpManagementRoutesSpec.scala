...
class ClusterHttpManagementRoutesSpec
  ...
  "Http Cluster Management Routes" should {
    ...
    "return cluster-wide sharding stats" when {
      "calling GET /cluster/shards/{name}/stats" in {
        import scala.concurrent.duration._
        ...
        val configClusterHttpManager = ConfigFactory.parseString(
          """
            |akka.management.http.hostname = "127.0.0.1"
            |akka.management.http.port = 20101
          """.stripMargin
        )
        ...
        implicit val t: ScalatestTimeout = ScalatestTimeout(15.seconds)

        shardRegion.ask("hello")(Timeout(3.seconds)).mapTo[String].futureValue(t)

        val clusterHttpManagement = ClusterHttpManagementRouteProvider(system)
        val settings = ManagementRouteProviderSettings(selfBaseUri = "http://127.0.0.1:20101", readOnly = false)
        val binding = Http().newServerAt("127.0.0.1", 20101).bind(clusterHttpManagement.routes(settings)).futureValue

        val responseGetClusterStats =
          Http().singleRequest(HttpRequest(uri = s"http://127.0.0.1:20101/cluster/shards/$name/stats")).futureValue(t)
        responseGetClusterStats.entity.getContentType shouldEqual ContentTypes.`application/json`
        responseGetClusterStats.status shouldEqual StatusCodes.OK
        val unmarshaledClusterStats =
          Unmarshal(responseGetClusterStats.entity).to[ClusterShardingStatsResponse].futureValue
        unmarshaledClusterStats.totalEntities shouldEqual 1
        unmarshaledClusterStats.totalShards shouldEqual 1
        unmarshaledClusterStats.regions should not be empty

        val responseInvalidGetClusterStats = Http()
          .singleRequest(
            HttpRequest(uri = s"http://127.0.0.1:20101/cluster/shards/ThisShardRegionDoesNotExist/stats")
          )
          .futureValue
        responseInvalidGetClusterStats.status shouldEqual StatusCodes.NotFound
    ...
    "return shard region state with entity IDs" when {
      "calling GET /cluster/shards/{name}/state" in {
        import scala.concurrent.duration._
        ...
        val configClusterHttpManager = ConfigFactory.parseString(
          """
            |akka.management.http.hostname = "127.0.0.1"
            |akka.management.http.port = 20102
          """.stripMargin
        )
        ...
        val clusterHttpManagement = ClusterHttpManagementRouteProvider(system)
        val settings = ManagementRouteProviderSettings(selfBaseUri = "http://127.0.0.1:20102", readOnly = false)
        val binding = Http().newServerAt("127.0.0.1", 20102).bind(clusterHttpManagement.routes(settings)).futureValue

        val responseGetShardState =
          Http().singleRequest(HttpRequest(uri = s"http://127.0.0.1:20102/cluster/shards/$name/state")).futureValue(t)
        responseGetShardState.entity.getContentType shouldEqual ContentTypes.`application/json`
        responseGetShardState.status shouldEqual StatusCodes.OK
        val unmarshaledShardState =
          Unmarshal(responseGetShardState.entity).to[ShardRegionStateResponse].futureValue
        unmarshaledShardState.shards should not be empty
        unmarshaledShardState.shards.head.shardId shouldEqual "ShardId"
        unmarshaledShardState.shards.head.entityIds should contain("1")

        val responseInvalidGetShardState = Http()
          .singleRequest(
            HttpRequest(uri = s"http://127.0.0.1:20102/cluster/shards/ThisShardRegionDoesNotExist/state")
          )
          .futureValue
        responseInvalidGetShardState.status shouldEqual StatusCodes.NotFound
