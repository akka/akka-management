...
private[akka] object ClusterHealthCheckRoutes {
   ...
   */
  @InternalApi
  private[akka] def memberStatus(status: String): MemberStatus = Helpers.toRootLowerCase(status) match {
  ...
  private[akka] class ClusterHealthCheckSettings(config: Config) {
