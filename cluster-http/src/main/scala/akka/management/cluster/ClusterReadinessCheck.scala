...
final class ClusterReadinessCheck private[akka] (system: ExtendedActorSystem, selfStatus: () => MemberStatus, settings: ClusterReadinessCheckSettings) extends (() => Future[Boolean]) {

  // TODO Java API for using directly
  def this(system: ExtendedActorSystem) = this(system, () => Cluster(system).selfMember.status,
